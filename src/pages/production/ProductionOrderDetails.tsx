...
interface OrderItem {
    ...
    unit_cost: number;
    total_cost: number;
    semi_finished?: { name: string; unit: string };
}
...
export default function ProductionOrderDetails() {
    ...
    const { data: items } = useQuery({
        queryKey: ['production-order-items', id],
        queryFn: async () => {
            const { data, error } = await supabase
                .from('production_order_items')
                .select('*, semi_finished:semi_finished_products!semi_finished_id(name, unit)')
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl sm:text-2xl font-bold">أمر إنتاج #{order.code}</h1>
                                        ...
                                        <TableRow key={item.id}>
                                            <TableCell className="font-medium">
                                                <div className="flex items-center gap-2">
                                                    <Beaker className="h-4 w-4 text-muted-foreground" />
                                                    {item.semi_finished?.name || `منتج #${item.semi_finished_id}`}
                                                </div>
                                            </TableCell>
                                            <TableCell>
